   ...
   <body>
      <div style="max-width:640px; margin:0 auto;">
         ...
         <div style="max-width:620px; border:1px solid #f0f0f0; margin:0 0; padding:15px; ">
            <h1 style="font-family:Arial; font-size:16px; font-weight:500; /*color:#8ccd56;*/ margin:5px 0 12px 0;">Hey {{@$mailData['name']}},</h1>
            ...
            <div style="display:block;overflow:hidden; width:100%; margin: 5px 0px 10px 0px;">
               ...
               <p>Thank you for registering in : {{@$mailData['course']}}</p>
               <p>You have successfully completed your payment: <b>{{@$mailData['price']}}</b> with transaction ID: {{@$mailData['t_id']}} <p>
                    
                    {{-- <br> 
                    <br> Name: {{@$mailData['name']}} 
                    <br> Email: {{@$mailData['email']}}
                    <br> Course: {{@$mailData['course']}}
                     <br> subject: {{@$mailData['subject']}}
                      <br> paper:  {{@$mailData['paper']}}
                       <br> amount: {{@$mailData['price']}}
                       <br>
                   <center>Payment success.!</center> --}}
                   <p>Now, you will get your User ID and Password shortly.<br>
                    Please visit the link for the app – <br>
                      https://play.google.com/store/apps/details?id=com.darkhorse.app
                      <br>
                      Kindly go through the app and login with these credentials to start preparing your
                       {{@$mailData['subject']}}and taking a daily test of  {{@$mailData['paper']}}.
                      Let’s make your learning more efficient.</p>
